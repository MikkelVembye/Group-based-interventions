...
group_based_dat <- readRDS("ES calc/Group-based interventions data.RDS") |> 
  ...
  mutate(
    ...
    eppi_id = if_else(author_year == "McCay et al. 2006", 105904715, eppi_id)
  )
...
# Check studies IDs. MHV: Hvor kommer studyid fra?

group_based_dat |> 
  summarise(
    n_eppi_ids = n_distinct(eppi_id),
    .by = studyid
  ) |> 
  filter(n_eppi_ids > 1)

group_based_dat |> filter(studyid %in% c(113, 120, 2) ) |> arrange(studyid)|> View()
...
GBD_effect <- 
 ...
 # MHV: I og med at vi ikke benytter unused outcomes, så kan disse slettes
...
>>>>>>> 9a77c09fc8c17b0cb994434cd2d8c199da36108f
```{r}
# Study context
