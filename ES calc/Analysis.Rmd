...
G_timeplot <- group_based_dat |> 
  summarise(year = unique(year), .by =  author_year)

# DONE
timeline_plot <-  ggplot(G_timeplot, aes(x = year)) + 
...
# Number of studies and effect size {.tabset}
...
GBD_effect |> 
  ...
  summarise(
    n_es = n(),
    .by = c(analysis_plan, eppi_id)
  ) |> 
  summarise(
    N_studies = length(eppi_id),
    N_es = sum(n_es),
    .by = analysis_plan
  ) |> 
  rename(
...
# We filtered out Employment, Physical health, and loneliness outcomes, since
# only few studies reported results for these outcomes. See above
...
## Reintegrational/social outcomes {.tabset}
```{r, echo=FALSE}
...
p_rob2 <- rob_summary(data = RoB2_dat, tool = "ROB2", overall = TRUE); p_rob2
...
p_robins <- rob_summary(data = ROBINS1_dat, tool = "ROBINS-I", overall = TRUE); p_robins
...
rho_beta_plot_abuse_misuse <- 
  ...
  scale_y_continuous(breaks = seq(-1, 1, .1), limits = c(-0.2, 0.4)) +
  theme_light() +
  theme(
...
rho_var_plot_abuse_misuse <- 
  ...
  scale_y_continuous(breaks = seq(-1, 1, .05), limits = c(-0.2, 0.4)) +
  scale_color_brewer(type = "qual", palette = 2) +
  geom_hline(yintercept = 0) +
  geom_vline(xintercept = 0.7, linetype = "dashed") +
  facet_grid(~param) +
...
# Evidence and gap map {.tabset}
```{r, echo=FALSE}
egm_dat <- 
  GBD_effect |>
  mutate(
    n_timing = case_when(
      timing %in% c("Posttest", "post", "Post") ~ "Post intervention (less than 1 year)",
      timing %in% c("3m", "3 months", "6w", "3") ~ " Follow-up (1-3 months)",
      timing %in% c("6m", "6 months", "9m") ~ "Follow-up (3-12 months)",
      timing %in% c("12m", "12", "18m", "24") ~ "Follow-up (Above 1 year)"),
    
    intervention_group = case_when(
        trt_type %in% c(
          "Group based Cognitive Behavioural Therapy",
          "Cognitive-Behavioral Social Skills Training",
          "Group Psychoeducation & Social Skill Training",
          "Group Psychoeducation"
        ) ~ "Psychotherapy and Cognitive Treatment",
        trt_type %in% c(
        ...
        trt_type %in% c(
          "Illness Management",
          "Illness and Addiction Management",
          "Addiction Management"
        ) ~ "Addiction and Illness Management",
        trt_type %in% c(
          "Art therapy",
          "Reading group intervention"
        ) ~ "Creative and Alternative Therapies",
        trt_type == "Vocational training" ~ "Training and Skill Development",
        trt_type == "Residential treatment" ~ "Treatment and Rehabilitation"
      )
    ) |>  
  summarise(
    n = n(),
    n_studies = n_distinct(Author), level = c(1:11),
    .by = c(intervention_group, analysis_plan, n_timing)
  ) |> 
  mutate(constant = "Intervention")


```

## Reintergrational outcomes
```{r, echo=FALSE}
egm_reintergration_plot <- ggplot(
  data = egm_dat |> filter(!str_detect(analysis_plan, "mental|used|hospi|Employment|Physical")),
  aes(x = n_timing, 
      y = constant, 
      size = n, 
      color = n_studies)
  ...
  facet_grid(intervention_group ~ analysis_plan, scales = "free_y", space = "free") +
  theme_bw() + 
  theme(
 ...
 # coord_cartesian(xlim = c(1, 4), ylim = NULL) +
  guides(size = "none") + 
  xlab("Timing (years)") +
  labs(color = "Number of studies"); egm_reintergration_plot
```

## Mental helath outcome
```{r, echo=FALSE}
egm_mental_health_plot <- ggplot(
  data = egm_dat |> filter(str_detect(analysis_plan, "mental")),
  aes(x = n_timing, 
      y = constant, 
      size = n, 
      color = n_studies)
 ...
 # coord_cartesian(xlim = c(1, 4), ylim = NULL) +
  guides(size = "none") + 
  xlab("Timing (years)") +
  labs(color = "Number of studies"); egm_mental_health_plot
```
