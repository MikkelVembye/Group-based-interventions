...
Acarturk2022_est <- 
  ...
  mutate(
    ...
    analysis_plan = case_when(
      outcome == "HSCL-25" ~ "Mental health outcomes",
      outcome == "PCL-5" ~ "Mental health outcomes",
  ...
  mutate(
    analysis_plan = rep(
      c("Hope, Empowerment & Self-efficacy", "Wellbeing and Quality of Life", 
        "Mental health outcomes/Hope, Empowerment & Self-efficacy"), 
      c(2, 1, 1))
  ) |> 
  rowwise() |> 
  mutate(
...
Bond2015_continious <- 
  tibble(
    ...
    analysis_plan = c("Mental health outcomes/Hope, Empowerment & Self-efficacy", "Psychiatric hospitalization"),
...
Bækkelund2022_est <-           
  ...
  mutate(
    ...
    analysis_plan = case_when(
      ...
      str_detect(outcome, "SCL-90 R") ~ "Mental health outcomes"),
    
    main_es_method = "diff-in-diffs",
    
    mean_z =  (w_t*z_t + w_c*z_c)/(w_t + w_c),  
    pp_cor = (exp(2*mean_z)-1)/(exp(2*mean_z)+1),
    pp_cor_methods = "Calculated from study results",
    ...
    mean_diff = if_else(outcome != "GAF-S", (diff_t - diff_c)*-1,
                        diff_t - diff_c),
    ...
    vd_DD = (1/N_t + 1/N_c) * 2*(1-pp_cor) + d_DD^2/(2*df_ind), # skaber falsk korrelation, hvorfor vi beregner det store variansled
    ...
    vg_DD = (1/N_t + 1/N_c) * 2*(1-pp_cor) + g_DD^2/(2*df_ind), # variansen af den baseline adjusted effekstørrelse
    Wg_DD = Wd_DD,
  ...
  mutate(
    ...
    n_group = 9,
    n_method = "Obtained from article (p. 4)",
    ...
    gamma_sqrt = VIVECampbell::gamma_1armcluster(
      N_total = N_total, Nc = N_c, avg_grp_size = n_group, ICC = icc, sqrt = TRUE
    ),
    
    h = df_h_1armcluster(N_total = N_total, ICC = icc, N_grp = N_t, avg_grp_size = n_group),
    omega = 1 - 3/(4*h-1),
  ...
  arrange(outcome, desc(timing)); Bækkelund2022_est
...
Cano_vindel2021_est <- 
  ...
  mutate(
    analysis_plan = case_when(
      outcome == "GAD-7" ~ "Mental health outcomes/Anxiety",
      outcome == "PHQ-9" ~ "Mental health outcomes/Depression",
      outcome == "PHQ15" ~ "Mental health outcomes",
...
Craigie2009_est <- 
  ...
  analysis_plan = rep(c("Mental health outcomes/Depression", "Mental health outcomes/Anxiety", 
...
Crawford2012_est <- 
  ...
  mutate(
    analysis_plan = case_when(
      str_detect(outcome, "Positive") ~ "Mental health outcomes/Symptoms of psychosis",
...
Dyck2000_est <-           
  Dyck2000_wide |>
  mutate(
    analysis_plan = "Mental health outcomes/Negative Symptoms",
...
Timmermans2012_est <- 
  ...
  mutate(vary_id = paste0(outcome, "/", timing),
         analysis_plan = case_when(
           ...
           outcome == "Self-e"  ~ "Mental health outcomes", 
...
Gatz2007_est <- 
  ...
  mutate(
    ...
    analysis_plan = c(
      ...
      "Mental health outcomes", "Mental health outcomes", "Hope, Empowerment & Self-efficacy"
    ),
...
Gonzalez2007 <- 
  tibble(
    ...
    Nt = 8, # Three patients terminated the group early (p. 411)
    Nc = 9, # Two of the control subjects were also lost two follow-up (p. 412)
    d = c(0.20, 0.66),
    p_val = c(
      0.65, 
    ...
    p_val_est = pf(F_val, df1 = 1, df2 = 11, lower.tail = FALSE)
...
Gordon2018_est <- 
  ...
  mutate(
    ...
    analysis_plan = case_when(
      outcome == "DASS-21" ~ "Mental health outcomes",
...
analysis_plan = case_when(
      str_detect(outcome, "PSS") ~ "Mental health outcomes",
      str_detect(outcome, "WHOQOL-BREF") ~ "Wellbeing and Quality of Life"),
...
Hagen2005_est <-           
  ...
  mutate(
    ...
    analysis_plan = case_when(
      str_detect(outcome, "SCL-90") ~ "Mental health outcomes",
      str_detect(outcome, "BAI") ~ "Mental health outcomes/Anxiety",
      str_detect(outcome, "BDI") ~ "Mental health outcomes/Depression",
...
Haslam2019_est <- tibble(
  ...
  analysis_plan = case_when(
      ...
      str_detect(outcome, "Depression") ~ "Mental health outcomes/Depression",
      str_detect(outcome, "Social Anxiety") ~ "Mental health outcomes",
...
Hilden2021_wide <-
  Hilden2021 |> 
  mutate(group = case_match(group, "Schema therapy group" ~ "t", 
                            "TAU" ~ "c")) |> 
  tidyr::pivot_wider(
    names_from = group,
    names_glue = "{.value}_{group}",
...
Hilden2021_est <-           
  ...
  mutate(
    analysis_plan = case_when(
      str_detect(outcome, "BSL-23 total") ~ "Mental health outcomes",
      str_detect(outcome, "Overall Anxiety Severity and Impairment scale") ~ "Mental health outcomes/Anxiety",
      str_detect(outcome, "Patient Health Questionnaire 9 scale") ~ "Physical health",
      str_detect(outcome, "Alcohol Use Disorders Identification Test scale") ~ "Alcohol and drug abuse/misuse",
      str_detect(outcome, "Sheehan disability work or studylife|Sheehan disability social life|Sheehan disability family life") ~ "Social functioning (degree of impairment)",
      str_detect(outcome, " Alcohol Use Disorders Identification Test scale") ~ "Alcohol and drug abuse/misuse",
      .default = NA_character_
    ),
    
    study = c("Hilden et al. 2021")
    ) |> 
...
timing = rep(c("Post","3m"), each = 12,1), #den respektive opfølgning skal gentages 18 gange
  ...
  N = rep(c(29, 29), each = 1, 12), #i hver gruppe er der 29 observationer
...
Himle2014_est <- 
  pmap(params, wide_Himle2014_func) |>
  list_rbind() |> 
  mutate(
    analysis_plan = rep(c(
    ...
    analysis_plan = case_when(
      str_detect(outcome, "LSAS -Total|Mini social phobia inventory|Brief fear of negative evaluation") ~ "Mental health outcomes",
      str_detect(outcome, "Beck anxiety inventory") ~ "Mental health outcomes/Anxiety",
      str_detect(outcome, "Sheehan disability scale") ~ "Social functioning (degree of impairment)",
      str_detect(outcome, "PHQ9 depression screen") ~ "Physical health",
      .default = NA_character_
    ),
    
    
    study = c("Himle et al. 2014")
  ) |> 
  mutate(
  ...
  mutate(
    analysis_plan = rep(
      c("Hope, Empowerment & Self-efficacy",
        "Social functioning (degree of impairment)",
        "Social functioning (degree of impairment)",
        "Mental health outcomes" # Total score
      ), each = 1,3),
...
```{r James et al. 2004}
James2004 <- tibble(
  group = rep(c("Treatment", "Control"), each = 1,4),
 
  outcome =  rep(c("BPRS", "DAST", "AUDIT","SDS"
  ), each = 2,1), 
...
James2004_est <- James2004_wide |> 
  mutate(
    ...
    analysis_plan = case_when(
      str_detect(outcome, "BPRS") ~ "Mental health outcomes",
...
Kanie2019_est <- 
  ...
  mutate(
    ...
    analysis_plan = case_when(
      ...
      str_detect(outcome, "BACS") ~ "Mental health outcomes",
      str_detect(outcome, "SFS|GAF") ~ "Social functioning",
      str_detect(outcome, "PANSS") ~ "Symptoms of psychosis",
      .default = NA_character_
    ),
...
Lim2020_est <-           
  ...
  mutate(
    ...
    analysis_plan = case_when(
      str_detect(outcome, "QLS") ~ "Wellbeing and Quality of Life",
      str_detect(outcome, "Negative") ~ "Mental health outcomes/Symptoms of psychosis/negative symptoms",
      
      .default = "All mental health/Symptoms of psychosis"
    ),
...
lloyd_Evans2020_est <-           
  ...
  mutate(
    analysis_plan = case_when(
      str_detect(outcome, "PHQ-9") ~ "Mental health outcomes/Depression",
      str_detect(outcome, "GAD-7") ~ "Mental health outcomes/Anxiety",
...
Madigan2013 <- tibble(
  ...
  arrange(outcome) |> 
relocate(outcome, .after = group); Madigan2013
...
# Multi level effect estimates can be obtained from table 3, p. 3
Madigan2013_est <- Madigan2013_wide |> 
  mutate(
    analysis_plan = case_when(
      str_detect(outcome, "ASI") ~ "Alcohol and drug abuse/misuse",
      str_detect(outcome, "Depressive symptoms") ~ "Depression/Mental health outcomes",
      str_detect(outcome, "Positive symptoms|Negative symptoms") ~ "Mental health outcomes",
      str_detect(outcome, "Global functioning") ~ "Social functioning (degree of impairment)",
...
Michalak_2015_MBCT_wide <- 
  Michalak_2015 |> 
  filter(!str_detect(group, "CBASP")) |> 
  mutate(group = case_match(group, "MBCT" ~ "t", "TAU" ~ "c")) |> 
  tidyr::pivot_wider(
    names_from = group,
    names_glue = "{.value}_{group}",
    values_from = N:last_col()
...
Michalak_2015_CBASP_wide <- 
  Michalak_2015 |> 
  filter(!str_detect(group, "MBCT")) |> 
  mutate(group = case_match(group, "CBASP" ~ "t", "TAU" ~ "c")) |> 
  tidyr::pivot_wider(
    names_from = group,
    names_glue = "{.value}_{group}",
    values_from = N:last_col()
...
Michalak_2015_est <- 
  bind_rows(Michalak_2015_MBCT_wide, Michalak_2015_CBASP_wide) |> 
  arrange(outcome) |> 
  mutate(
    analysis_plan = rep(c(
...
Michalak2015_est2 <- 
  tibble(
    ...
    outcome = c("SF-36","SF-36", "SF-36", "SF-36", "SF-36","SF-36", "SF-36", "SF-36",
                "SASS", "SASS"),
    subscale = rep(c("Vitality", "Social func", "Role", "Mental health", "overall"), each = 2,1),
...
Morley2014_est <-           
  ...
  mutate(
     ...
     analysis_plan = case_when(
      str_detect(outcome, "BSS") ~ "Mental health outcomes",
      str_detect(outcome, "HADS_depression") ~ "Mental health outcomes/Depression",
      str_detect(outcome, "HADS_anxiety") ~ "Mental health outcomes/Anxiety",
...
#  t-values based on ANOVA and taken from text (pp. 357-359)
# But gives presumably wrong estimates. Why i use imputed ppcor
...
# r = ((sd_pre^2*t_val + sd_post^2*t_val) - 
#        (m_post-m_pre)^2 * N)/ (2 * sd_pre*sd_post*t_val),
...
Morton2012_est1 <- 
  ...
  #  pp_cor_methods = "Calculated from study results",
    ...
    #ppcor imputed for DASS: depression, and DASS: stress,
  ppcor = ppcor_imp,
  pp_cor_methods = "Imputed"
...
Morton2012_est <- bind_rows(
  ...
  mutate(
    study = "Morton et al. 2012",
     ...
     analysis_plan = case_when(
      str_detect(outcome, "BEST_composite|DASS_anxiety|DASS_stress|DASS_depression") ~ "Mental health outcomes",
      str_detect(outcome, "BHS") ~ "Hope, Empowerment & Self-efficacy",
      str_detect(outcome, "DERS_total") ~ "Mental health outcomes/Unused outcomes",
      .default = NA_character_
    ),
...
Patterson2003_wide  <- Patterson2003 |> 
...
Patterson2003_est <- 
  ...
  mutate(
     analysis_plan = case_when(
      str_detect(outcome, "PANSS_General|PANSS_Negative|PANSS_Positive") ~ "Mental health outcomes/Symptoms of psychosis",
      str_detect(outcome, "HAM_D") ~ "Mental health outcomes/Depression",
  ...
  mutate(
    ...
    m_post = if_else(analysis_plan %in% c("Mental health outcomes/Symptoms of psychosis",
                                     "Mental health outcomes/Depression"), 
                     (m_post_t - m_post_c)*-1,
                     m_post_t - m_post_c),
   ...
   m_diff_t = if_else(analysis_plan %in% c("Mental health outcomes/Symptoms of psychosis",
                                     "Mental health outcomes/Depression"), 
                      (m_post_t - m_pre_t)*-1,
                      m_post_t - m_pre_t),
    m_diff_c = if_else(analysis_plan %in% c("Mental health outcomes/Symptoms of psychosis",
                                     "Mental health outcomes/Depression"),
                       (m_post_c - m_pre_c)*-1,
                       m_post_c - m_pre_c),
...
Popolo2019_est <- 
  ...
  mutate(
    ...
    analysis_plan =
      c("Mental health outcomes", 
        "Mental health outcomes/Unused outcomes", 
        "Mental health outcomes"),
  ...
  mutate(
    ...
    h = df_h_1armcluster(N_total = N_total, ICC = icc, N_grp = N_t, avg_grp_size = avg_cl_size),
    omega = 1 - 3/(4*h-1),
...
r = ((sd_pre^2*t_val^2 + sd_post^2*t_val^2) - 
        (m_post-m_pre)^2 * N)/ (2 * sd_pre*sd_post*t_val^2),
...
Rosenblum2014_est <- 
  ...
  #            0.01, 0.10, 0.31, 0.06, 0.15),
    ...
    analysis_plan = case_when(
      str_detect(outcome, "Days any alcohol past 30|Days heavy alcohol past 30|Days any drugs past 30|Days any drugs or alcohol past 30") ~ "Alcohol and drug abuse/misuse",
...
#    21, 23, 
  ...
  arrange(outcome, timing); Russinova2018
...
Russinova2018_est <- Russinova2018_wide |> 
  ...
  #  ),
    ...
    analysis_plan = case_when(
      str_detect(outcome, "Empowerment scale|Work Hope Scale|Work motivation") ~ "Hope, Empowerment & Self-efficacy",
...
# To also calculate pooled data to get combined estimates for both high and low/medium propensity 
calculate_pooled_estimates <- function(N, means, sds) {
  weighted_mean <- sum(N * means) / sum(N)
  sum_squared_differences <- sum(N * (means - weighted_mean)^2)
  sum_variances <- sum((N - 1) * sds^2)
  total_N <- sum(N)
  pooled_sd <- sqrt((sum_variances + sum_squared_differences) / (total_N - 1))
  return(pooled_sd)
}


# Calculate combined statistics
Sacks2011_total <- Sacks2011 %>%
  group_by(group, outcome) %>%
  summarise(
    sample = "total",
    N_total = sum(N),
    m_pre = weighted.mean(m_pre, N),
    sd_pre = calculate_pooled_sd(N, m_pre, sd_pre),
    m_post = weighted.mean(m_post, N),
    sd_post = calculate_pooled_sd(N, m_post, sd_post),
    .groups = 'drop'
  ) |> 
  rename(N = N_total) |> 
  arrange(outcome, desc(group))



Sacks2011 <- bind_rows(Sacks2011, Sacks2011_total)
...
Sacks2011_est <- 
  ...
  mutate(
   ...
   analysis_plan = case_when(
     outcome == "BDI_total" ~ "Mental health outcomes/Depression",
      outcome == "GSI_total" ~ "Mental health outcomes",
...
Sajatovic2009_wide <- Sajatovic2009 |>
...
Sajatovic2009_est <- Sajatovic2009_wide |> 
  mutate(
    ...
    analysis_plan = rep(
    c("Mental health outcomes/Depression", # HAM-D
      "Mental health outcomes", # YMRS
...
Schrank2016_est <-           
  ...
  mutate(
    analysis_plan = case_when(
      str_detect(outcome, "WEMWBS|MANSA") ~ "Alcohol and drug abuse/misuse",
      str_detect(outcome, "BPRS") ~ "Mental health outcomes",
      str_detect(outcome, "IHS|RSES|RES") ~ "Hope",
      .default = NA_character_
    ),
...
schafer2019_est <- 
  ...
  mutate(
    analysis_plan = case_when(
      str_detect(outcome, "ASI alcohol severity|ASI drug severity|Drug and alcohol free days") ~ "Alcohol and drug abuse/misuse",
      str_detect(outcome, "BDI-II") ~ "Mental health outcomes/Depression",
      str_detect(outcome, "DERS") ~ "Unused outcomes",
      str_detect(outcome, "PDS|PSS-I") ~ "Mental health outcomes",
      .default = NA_character_
    ),
...
Rüsch2019_est <- Rüsch2019_wide |> 
  mutate(
    
    analysis_plan = case_when(
      str_detect(outcome, "CES-D") ~ "Mental health outcomes/Depression",
...
Tjaden2021_est <- Tjaden2021_wide |> 
  ...
  mutate(
    analysis_plan = case_when(
      str_detect(outcome, "BSI") ~ "Mental health outcomes",
  ...
  # Effect sizes are acoress different time    
  #  d = c(
...
Valiente2022_est <-           
  ...
  mutate(
    ...
    m_post = if_else(analysis_plan != "Mental health outcomes", m_post_t - m_post_c,
                     (m_post_t - m_post_c) * -1),
    ...
    m_diff_t = if_else(analysis_plan != "Mental health outcomes", m_post_t - m_post_c, (m_post_t - m_pre_t)*-1),
    m_diff_c = if_else(analysis_plan != "Mental health outcomes", m_post_c - m_pre_c, (m_post_c - m_pre_c)*-1),
...
Wojtalik2019_est <- Wojtalik2019_wide |> 
  mutate(
    analysis_plan = case_when(
    ...
    # There is not any within group t-test
    #t_val = c(
    # ITT
