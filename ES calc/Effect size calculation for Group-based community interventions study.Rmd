...
Craigie2009_est <- 
  ...
  analysis_plan = rep(c("Mental health outcomes/Depression", "Mental health outcomes/Anxiety", "Wellbeing and Quality of Life"), 2),  
  effect_size = "SMD",
  sd_used = "Pooled posttest SD",
...
Haslam2019_est <- tibble(
 # analysis_plan = c(
#        "Roberts UCLA Loneliness Scale",
#        "Depression, Anxiety, and Stress Scale-21 (the depression subscale)",
#        "Short version of the Social Phobia Inventory (mini-SPIN)",
#        "Single-item question about the number of visits to a general practitioner in the last month",
#        "Four-item multiple group membership scale"
#  ),
...
Hilden2021_est <-           
  ...
  mutate(
   # analysis_plan =
   #  c("the short Borderline Symptom List (BSL-23)",
...
Jacob2010_est <- 
  Jacob2010_wide |> 
  mutate(
    analysis_plan = if_else(outcome == "BDI", "Depression", "Self-esteem"), 
...
Kanie2019_est <- 
  ...
  mutate(
    ...
    analysis_plan = case_when(
      ...
      str_detect(outcome, "BACS") ~ "All mental health",
    ...
    #    "Global assessment of Functioning (Observer rated)", 
...
Lim2020_est <-           
  ...
  mutate(
    ...
    analysis_plan = case_when(
      str_detect(outcome, "QLS") ~ "Wellbeing and Quality of Life",
      str_detect(outcome, "Negative") ~ "All mental health/Symptoms of psychosis/negative symptoms",
...
Michalak_2015_est <- 
  ...
  mutate(
    analysis_plan = rep(c(
      "Depression: Hamilton Depression Rating Scale (BDI)",
  ...
  mutate(
    ...
    #  ),
     ...
     analysis_plan = case_when(
      ...
      str_detect(outcome, "HADS_depression") ~ "Depression/Mental health outcomes",
      str_detect(outcome, "HADS_anxiety") ~ "Anxiety/Mental health outcomes",
  ...
  mutate(
   ...
   #    "DASS depression")
     ...
     analysis_plan = case_when(
      ...
      str_detect(outcome, "DERS_total") ~ "Unused outcomes/Mental health outcomes",
      .default = NA_character_
    ),
...
Patterson2003_est <- 
  ...
  mutate(
     analysis_plan = case_when(
      str_detect(outcome, "PANSS_General|PANSS_Negative|PANSS_Positive") ~ "Symptoms of psychosis/Mental health outcome",
      str_detect(outcome, "HAM_D") ~ "Depression/Mental health outcomes",
  ...
  mutate(
    ...
    m_post = if_else(analysis_plan %in% c("Positive And Negative Syndromes Scale (PANSS)",
                                     "Hamilton Depression Rating Scale (HAM-D)"), 
                     (m_post_t - m_post_c)*-1,
                     m_post_t - m_post_c),
   ...
   m_diff_t = if_else(analysis_plan %in% c("Positive And Negative Syndromes Scale (PANSS)",
                                     "Hamilton Depression Rating Scale (HAM-D)"), 
                      (m_post_t - m_pre_t)*-1,
                      m_post_t - m_pre_t),
    m_diff_c = if_else(analysis_plan %in% c("Positive And Negative Syndromes Scale (PANSS)",
                                     "Hamilton Depression Rating Scale (HAM-D)"),
                       (m_post_c - m_pre_c)*-1,
                       m_post_c - m_pre_c),
  ...
  mutate(
    ...
    analysis_plan =
      c("Mental health outcomes", 
        "Unused outcomes/Mental health outcomes", 
...
Rabenstein2015_est <-           
  ...
  mutate(
   ...
   # The outcome BSI and BDI are reverted because lower score is beneficial  
    m_diff_t = if_else(analysis_plan %in% c("Brief Symptom Inventory", "Beck Depression Inventory (BDI)"),
                       (m_post_t - m_pre_t)*-1, m_post_t - m_pre_t),
    m_diff_c = if_else(analysis_plan %in% c("Brief Symptom Inventory", "Beck Depression Inventory (BDI)"),
...
Russinova2018_est <- Russinova2018_wide |> 
  ...
  #  ),
    ...
    analysis_plan = rep(c(
      "The Empowerment Scale (28-item)",
      "The Internalized Stigma of Mental Illness (ISMI 29-itm) scale",
      "The Work Hope Scale",
      "Vocational Identity Scale",
      "The Work Motivation Scale (12-item)"
    ), each = 1,2),
...
Schrank2016_est <-           
  ...
  mutate(
    ...
    # The outcome BPRS is reverted because lower score is beneficial
    m_diff_t = if_else(analysis_plan != "Brief Psychiatric Rating Scale (BPRS)", 
                      m_post_t - m_pre_t,
                      (m_post_t - m_pre_t)*-1),
    m_diff_c = if_else(analysis_plan != "Brief Psychiatric Rating Scale (BPRS)",
                       m_post_c - m_pre_c,
                       (m_post_c - m_pre_c)*-1),
...
schafer2019_est <- 
  ...
  mutate(
   # analysis_plan = rep(
  #    c(rep(c("Alcohol and drug abuse/misuse"), each = 6,1),
  #      rep(c("Alcohol and drug abuse/misuse"), each = 6,1),
  #      rep(c("Depression"), each = 6,1),
  #      rep(c("Unused outcomes"), each = 6,1),
  #      rep(c("Alcohol and drug abuse/misuse"), each = 6,1),
  #      rep(c("Mental health outcomes"), each = 6,1),
  #      rep(c("Mental health outcomes"), each = 6,1)
    ...
    analysis_plan = case_when(
      ...
      str_detect(outcome, "BDI-II") ~ "Depression",
...
Tjaden2021_est <- Tjaden2021_wide |> 
  ...
  mutate(
    # analysis_plan = rep(
    # c( "The NetherlandsEmpowerment List",
    #    "BriefSymptom Inventory-18",
