...
Crawford2012_act_wide <- 
  ...
  tidyr::pivot_wider(
    names_from = group,
    names_glue = "{.value}_{group}",
    values_from = N_baseline:last_col()
  ) |> 
  mutate(
...
test <- summary(lm(post_test_std ~ group + pre_test_std, data = filter(Gutnam2019_raw, outcome == "PSS")))
test$coefficients[2]
```
...
Gutnam2019 <- left_join(Gutnam2019, Gutnam2019_cor, by = join_by(outcome))

Gutnam2019_est <-
  Gutnam2019_raw |> 
  summarise(
    
    d_DD = summary(lm(post_test_std ~ group + pre_test_std, data = pick(everything())))$coefficients[2],
    var_term1_DD = summary(lm(post_test_std ~ group + pre_test_std, data = pick(everything())))$coefficients[5]^2,
  ...
  mutate(
    ...
    d_DD = if_else(outcome == "PSS", d_DD * -1, d_DD)
    
  ) |> 
  left_join(Gutnam2019, by = join_by(outcome)) |> 
  mutate(
  ...
  relocate(c(d_DD:var_term1_DD), .after = last_col()) |> 
  mutate(
    
  vd_DD = var_term1_DD + d_DD^2/(2*df_ind),
  Wd_DD = var_term1_DD,
  
  g_DD = J * d_DD, 
  vg_DD = var_term1_DD + g_DD^2/(2*df_ind),
  Wg_DD = var_term1_DD,
    
  avg_cl_type = "Not relevant - one cluster only",
  ...
  gt_post = g_post * sqrt(1-icc),
  vgt_post = vg_DD * (1-icc),
  Wgt_post = var_term1_DD * (1-icc),
