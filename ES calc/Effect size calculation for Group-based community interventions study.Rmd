...
# Prepost-test correlations between anxiety, depression and loneliness measure for the CBT group

ppcor_Wuchrich2021 <- 
  tibble(
    
    outcome = c("Loneliness", "Anxiety", "Depression"),
    
    group = "CBT",
    
    m_pre = c(1.7, 5.43, 5.67),
    sd_pre = c(0.82, .74, 1.05)
...
F_stat_Wuthrich <- 
  tibble(
    ...
    outcome = c("Primary problem", "Anxiety", "Depression", "GDS", "Loneliness", "GAI", "PSWQ", "SF12"),
    df1 = c(1),
    df2 = c(
...
Wuthrich2013_2021 <- 
  tibble(
    ...
    outcome = rep(c("Primary problem", "Anxiety", "Depression"), each = 2),
...
Wuthrich2013_2021_es <- 
  ...
  left_join(select(F_stat_Wuthrich[1:3,], -year)) |> 
  mutate(
    
    vd = 1/N_t + 1/N_c + d_DD^2/(2*N_tot),
    Wd = 1/N_t + 1/N_c,
    vd_alt = d_DD^2/F_val + d_DD^2/(2*N_tot),
    Wd_alt = d_DD^2/F_val,
