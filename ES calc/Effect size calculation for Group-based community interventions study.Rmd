...
wide_format_Timmermans2012 <- function(data, filter){
  ...
  filter_func <- function(data, filter){
    ...
    dat <- data |> dplyr::filter(measure != filter)
...
Timmermans2012_wide <- 
wide_format_Timmermans2012(data = Timmermans2012, filter = c("followup", "post"))
...
Timmermans2012_es <- 
  ...
  mutate(
    ...
    # Make alternative and posttest
    
    
  ) |> 
  ungroup(); Timmermans2012_es
...
Wuthrich2013_2021_es_smd <- 
  ...
  summarise(
    ...
    d = DD_emm/s_pool,
    vd = sum(1/N) + d^2/(2*N_tot),
    Wd = sum(1/N),
    
    d_alt = b_adj_emm/s_pool,
    vd_alt = sum(1/N) + d_alt^2/(2*N_tot),
    Wd_alt = sum(1/N),
    
    J = 1 - 3/(4*N_tot-1),
    
    g = J * d,
    vg = sum(1/N) + g^2/(2*N_tot),
    Wg = Wd,
    
    g_alt = J * d_alt,
    vg_alt = sum(1/N) + g_alt^2/(2*N_tot),
    Wg_alt = sum(1/N),
