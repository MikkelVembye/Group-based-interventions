...
````{r Jacob et al. 2010}
Jacob2010 <- tibble(group = as.factor(rep(c("Intervention group",
    "Control group"), 
    each = 1, 10)), 
...
# Turning data into wide format
params <- tibble(
  filter_val1 = rep(c("Post", paste0(c(6), "m")), 1)
...
# MANCOVA estimates can be obtained from the text on p. 382
Jacob2010_est <- 
  pmap(params, wide_Jacob2010_func) |>
  list_rbind() |> 
  mutate(
    analysis_plan = rep(c(
...
Kanie2019_est <- 
  pmap(params, wide_Kanie2019_func) |>
  list_rbind() |>
  # Based on the degrees of freedom value reported in Druss et al. 2018 table 2 and 3
  mutate(
    analysis_plan = rep(
...
# Turning data into wide format
params <- tibble(
  filter_val1 = rep(c("Post", paste0(c(3, 12), "m")), 1)
  
)



wide_Madigan2013_func <- 
  function(filter_val1){
...
# Multi level effect estimates can be obtained from table 3, p. 3
Madigan2013_est <- 
  pmap(params, wide_Madigan2013_func) |>
  list_rbind() |> 
  mutate(
    analysis_plan = rep(c(
