...
wide_format_Timmermans2012 <- function(data, filter){
  ...
  filter_func <- function(data, filter){
    ...
    tidyr::pivot_wider(
      names_from = c(group, measure),
      names_glue = "{.value}_{group}_{measure}",
      values_from = N_start:se_b_adj
    )
...
Wuthrich2013_2021_es_smd <- 
  ...
  mutate(
    ...
    gamma_sqrt_005 = gamma_1armcluster(
      N_total = N_tot, Nc = N_c, avg_grp_size = avg_cl_size, ICC = ICC_005
      ),
    gamma_sqrt_02 = gamma_1armcluster(
