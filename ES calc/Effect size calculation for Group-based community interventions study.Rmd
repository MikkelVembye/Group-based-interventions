...
# for the effect size correlation
  ...
  df1 = 1, 
  df2 = 31,
  F_val = qf(p_val_f/2, df1 = df1, df2 = df2, lower.tail = FALSE)
  )
```
...
Druss2010_wide <- 
  ...
  mutate(
    N_total = N_t + N_c,
    p_val_baseline_diff = c(0.96, 0.88, 0.78, 0.98, 0.80), # From Table 2 (p. 268)
    p_val_post_diff = c(0.01,0.14, 0.25, 0.27, 0.95), # From Table 2 (p. 268)
    ...
    p_val_f = c(0.03, 0.4, 0.22, 0.41, 0.96),
    F_val = qf(p_val_f/2, df1 = 1, df2 = N_total - 2, lower.tail = FALSE),
...
test_dat <- covariates |> 
  filter(str_detect(authors, "Cano")) |> 
  bind_cols(Cano_vindel2021_est)
