...
Druss2018_est <- 
  ...
  mutate(
    ...
    d_DD = (m_diff_t - m_diff_c)/sd_pool,
    vd_DD = d_DD^2/ppcor + d_DD^2/(2*df_ind),
    Wd_DD = d_DD^2/ppcor,
    
    g_DD = J * d_DD, 
    vg_DD = g_DD^2/ppcor + g_DD^2/(2*df_ind),
    Wg_DD = g_DD^2/ppcor,
...
### **Jacob et al. [-@Jacob2010]**
````{r Jacob et al. 2010}
Jacob2010 <- tibble(group = as.factor(rep(c("Intervention",
    "Control"), 
    each = 1, 10)), 
  
  timing = rep(c("Post","6m"), each = 10,1), #den respektive opfølgning skal gentages 18 gange
...
Jacob2010_est <- Jacob2010_wide |> 
  ...
  mutate(
   ...
   # Reverting m_post for specific outcomes
    m_post = if_else(outcome %in% c("BDI"),
                     (m_post_t - m_post_c) * -1, 
                     m_post_t - m_post_c),
    ...
    m_diff_t = if_else(outcome %in% c("BDI"),
                       (m_post_t - m_pre_t)*-1,
                        m_post_t - m_pre_t),
    m_diff_c = if_else(outcome %in% c("BDI"),
                       (m_post_c - m_pre_c)*-1,
                       m_post_c - m_pre_c),
...
Kanie2019 <- tibble(
  group = as.factor(rep(c(
    "SCIT group", 
    "TAU group"),
    each = 1,5)), # 10 rækker af hver gruppe, fordi der er fem outcomes og to opfølgningsperioder
  
  timing = rep(c(
    #"3m", 
    "post"), 
               each = 10,1), # Den respektive opfølgning skal gentages 10 gange
  ...
  ),  each = 2,1), # outcomes gentages to gange for hver opfølgningsperiode
  
  
  N = rep(c(32, 32), each = 1,5), # i hver gruppe er der 32 observationer
                                  # som skal gentages 10 gange for hver opfølgning
  ...
  sd_pre = rep(c(
    ...
    8.81, 9.49,
    23.95, 23.48)
    , each = 1,1), ## den gentager præ-værdierne ved næste opfølgning
                   # så den sammenligner baseline data med 6 måneders opfølgning
                  # så sørger vi for at der er lige mange kolonner
  
  m_post = c(
    # 3 months measurement - This is a midterm measurement 
  #  32.94, 33.45, 
  #  -1.37, -1.16, 
  #  61.34, 59.94, 
  #  52.25, 58.06,
...
Kanie2019_wide <- Kanie2019 |> 
      mutate(group = case_match(group, "SCIT group" ~ "t", "TAU group" ~ "c")) |>
      tidyr::pivot_wider(
        names_from = group,
        names_glue = "{.value}_{group}",
        values_from = N:last_col()
...
Kanie2019_est <- Kanie2019_wide |> 
  ...
  mutate(
    ...
    # For PANSS: reverted
    m_post = if_else(outcome != "PANSS total", 
                     (m_post_t - m_post_c)*-1,
                     m_post_t - m_post_c),
    sd_pool = sqrt(((N_t-1)*sd_post_t^2 + (N_c-1)*sd_post_c^2)/(N_t + N_c - 2)),  
    ...
    # For PANSS: reverted
    m_diff_t = if_else(outcome != "PANSS total",
                       (m_post_t - m_pre_t)*-1,
                        m_post_t - m_pre_t),
    m_diff_c = if_else(outcome != "PANSS total",
                       (m_post_c - m_pre_c)*-1,
                       m_post_c - m_pre_c),
  ...
  mutate(
    ...
    eta_sqrt = c(0.080, 0.125, 0.002, 0.123, 0.041, 0.191), # unsure if this should be included
      ...
      mutate(
       ...
       #  icc values obtained from p. 2
   
    icc = rep(c(0.98, # ICC for QLS, repeated once
                0.956), # ICC for PANSS, repeated five times
              c(1, 5)),
    
    icc_type = "From study (p. 2)"
...
schafer2019_est <- 
  ...
  mutate(
    analysis_plan = rep(
      c(rep(c("the Addiction Severity Index-lite (ASI-lite)"), each = 6,1),
        rep(c("the Addiction Severity Index-lite (ASI-lite)"), each = 6,1),
        rep(c("Depression was assessed by the Beck Depression Inventory II (BDI-II)"), each = 6,1),
        rep(c("the Difficulties in Emotion Regulation Scale (DERS)"), each = 6,1),
        rep(c("the Addiction Severity Index-lite (ASI-lite)"), each = 6,1),
        rep(c("PTSD severity was self-reported by participants using the PDS"), each = 6,1),
        rep(c("the PTSD Symptom Scale Interview (PSS-I)"), each = 6,1)
  ...
  mutate(
    ...
    m_post = ((m_post_t - m_post_c) * -1), # For all scores: Lower is beneficial
...
# Turning data into wide format
params <- tibble(
   filter_val1 = rep(c(paste0(c(9, 18), "m")), 1)
)

wide_Tjaden2021_func <- 
  function(filter_val1){
...
Tjaden2021_est <- 
  pmap(params, wide_Tjaden2021_func) |>
  list_rbind() |>
  # Based on the degrees of freedom value reported in Druss et al. 2018 table 2 and 3
  mutate(
    analysis_plan = rep(
