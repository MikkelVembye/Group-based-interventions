...
Kanie2019 <- tibble(
  group = as.factor(rep(c(
    "SCIT group", 
    "TAU group"),
    each = 1,10)), # 10 rækker af hver gruppe, fordi der er fem outcomes og to opfølgningsperioder
  ...
  N = rep(c(32, 32), each = 1,10), # i hver gruppe er der 32 observationer
  ...
  sd_pre = rep(c(
    ...
    8.81, 9.49,
    23.95, 23.48)
    , each = 1,2), ## den gentager præ-værdierne ved næste opfølgning
...
# mangler effektudregning ----
params <- tibble(
  filter_val1 = rep(c("Post", paste0(c(3, 6), "m")), 1)
  
)

wide_schafer2019_func <- 
  function(filter_val1){
    
    schafer2019 |> 
      filter(timing == filter_val1) |> 
      mutate(group = case_match(group, "Seeking Safety" ~ "t1", "Relapse Prevention Training" ~ "t2" , "TAU" ~ "c",)) |>
      tidyr::pivot_wider(
        names_from = group,
        names_glue = "{.value}_{group}",
        values_from = N:last_col()
...
schafer2019_est <- 
  pmap(params, wide_schafer2019_func) |>
  list_rbind() |>
  ...
  mutate(
    analysis_plan = rep(
      c(
        ...
        "PTSD severity was self-reported by participants using the PDS",
        "the Addiction Severity Index-lite (ASI-lite)",
        "the Addiction Severity Index-lite (ASI-lite)",
        "the Addiction Severity Index-lite (ASI-lite)",
        "Depression was assessed by the Beck Depression Inventory II (BDI-II)",
...
group_based_dat_es <- 
  bind_rows(
    ...
    Gordon2018_est,
    Hagen2005_est,
#    Hilden_est,     # Not extracted to excel
#    Himle2014_est,  # Not extracted to excel
    izquierdo2021_est,
#    Jacob2010_est,  # Not extracted to excel
#    Kanie2019_est,  # Not extracted to excel
    Lim2020_est,
    lloyd_Evans2020_est,
#    Madigan2013_est, # Not extracted to excel
    Morley2014_est,
    Morton2012_est, 
    Patterson2003_est,
    Popolo2019_est,
    Rabenstein2015_est,
    Rosenblum2014_est,
#    Russinova2018_est, # Not extracted to excel
    Rüsch2019_est, 
    Sacks2011_est,
    Sajatovic2009_est,
    Schrank2016_est,
#    schafer2019, Mangle effektudregning
#    Tjaden2021_est # Not extracted to excel
    Valiente2022_est,
    Volpe2015_est,
    Wojtalik2019_est
