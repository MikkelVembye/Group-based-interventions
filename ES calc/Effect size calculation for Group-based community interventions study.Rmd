...
Entering data from Table 2 (p. 445). Sample size information is retrieved from Table 1 (p. 442). Below we also conduct a small number of clusters correction, as suggested by WWC [-@WWC_clusterbias].
...
Wuthrich2013_2021_es <- 
  ...
  mutate(
    ...
    gamma_sqrt = gamma_1armcluster(N_total = N_tot, Nc = N_c, avg_grp_size = avg_cl_size, ICC = icc),
...
Wuthrich2013_2021_smd_est <- 
  left_join(Wuthrich2013_2021_long, Wuthrich2013_2021_es) |> 
...
Wuthrich2013_OR_dat <- 
  tibble(
    ...
    ln_OR = log(OR),
    vln_OR = 1/(N_t*p_t) + 1/(N_t*(1-p_t)) + 1/(N_c*p_c) + 1/(N_c*(1-p_c)),
    vln_OR_C = 1/(N_t*p_t_cl) + 1/(N_t*(1-p_t_cl)) + 1/(N_c*p_c) + 1/(N_c*(1-p_c)),
    
  ); Wuthrich2013_OR_dat


# Amalgamating the all results
