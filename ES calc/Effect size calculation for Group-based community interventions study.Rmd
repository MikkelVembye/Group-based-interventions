...
Gonzalez2007 <- 
  tibble(
    ...
    b_emm_t = c(72.20, 1.75),
    b_emm_c = c(69.30, 2.61),
    
    se_emm_t = c(4.11, 0.37),
    se_emm_c = c(4.11, 0.37),
    
    b_emm_adj = (b_emm_t - b_emm_c),
    
    sd_total = abs(b_emm_adj)/es_paper,
    # Using E.14 from WWC Handbook (ver. 5). We imputed the covariate-adjusted mean difference to obtain this esti
    sd_pool = sqrt( ((N_total-1)/(N_total-2)) * sd_total^2 - ((N_t*N_c)/(N_total*(N_total-2))) * b_emm_adj^2),
...
### Should be cheked by MVH: adds a lot of new variables
```{r, echo=FALSE}
...
dat_test <- 
  dat |> 
  bind_rows(Wuthrich2013_2021_smd_est) |> 
  relocate(study)
```
