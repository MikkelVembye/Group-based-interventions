...
Timmermans2012_est <- 
  ...
  summarise(
    ...
    es_method = "Posttest only",
    
    ppcor = NA,  
    ppcor_type = "Not relevant",
...
### **Bond et al. [-@Bond1991]**
#### (No cluster treatment)

Entering data from page 39
...
Effect size calculation and cluster bias correction
...
Bond_1991_est <- 
  ...
  mutate(
    ...
    vd_pair = (1/n + (d_pair^2/(2*n))) * 2*(1-r), 
    Wd_pair = 1/n * (2*(1-r))
  ) |> 
  summarise(
    
    study = "Bond1991",
    es_method = "Pre-test adjusted",
    ...
    vd_pp_trt = c(vd_pair[1], vd_pair[2]),
    vd_pp_ctr = c(vd_pair[3]),
    
    Wd_pp_trt = c(Wd_pair[1], Wd_pair[2]),
    Wd_pp_ctr = c(Wd_pair[3])
    
  ) |> 
  mutate(
    
    d = d_pp_trt - d_pp_ctr,
    vd = (1/Nt + 1/Nc) * 2*(1-ppcor) + d^2/(2*(Nt+Nc)),
    # Insert alternative vd calculation based on Table 11.2 (Borenstein & Hedges 2019, p. 216)
    Wd = (1/Nt + 1/Nc) * 2*(1-ppcor),
    
    vd_alt = vd_pp_trt + vd_pp_ctr,
    Wd_alt = Wd_pp_trt + Wd_pp_ctr,
    ...
    n_group = 12.5, 
    icc = ICC_01, # See page 57 - needs to be checked how this should be interpreted
    icc_type = "Imputed",
    ...
    # Not alternative calculation due equivalent Wd and Wd_alt
    gt_alt = NA,
    vgt_alt = NA,
    Wgt_alt = NA,
