...
Patterson2003_est <- 
  ...
  mutate(
    ...
    m_post = if_else(outcome  %in% c("QWB","HAM_D"), (m_post_t - m_post_c)*-1,
                     m_post_t - m_post_c),
    sd_pool = sqrt(((N_t-1)*sd_post_t^2 + (N_c-1)*sd_post_c^2)/(N_t + N_c - 2)),  
...
params <- tibble(
  filter_val1 = rep(unique(Russinova2018$timing)
                    , each = 1
                    )
)

wide_Russinova2018_func <- 
  function(filter_val1){
...
Russinova2018_est <- 
  pmap(params, wide_Russinova2018_func) |>
  list_rbind() |> 
  mutate(
    # Group effect af 3 months from table 4, p. 203 
    df1 = c(
      rep(c(NA_real_), each = 5,1)
    ,
      rep(c(1), each = 5,1)),
