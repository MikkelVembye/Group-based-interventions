...
GBD_effect <- 
  ...
  mutate(
    ...
    vgt_post = if_else(is.na(vgt_post) & !is.na(vgt_DD), vg_DD, vgt_post),
    ...
    Wgt_post = if_else(is.na(Wgt_post) & !is.na(Wgt_DD), Wg_DD, Wgt_post),
...
GBD_effect |> mutate(invsample = (1/N_t + 1/N_c), prop_wgt = Wgt/invsample) |> filter(prop_wgt > 1.5, Wgt >= 0.1) |> select(author_year, Wgt, Wd, invsample, prop_wgt)
```
...
gb_dat <- 
  GBD_effect |> 
  filter(!c(str_detect(author_year, "Cano|Craigie") & analysis_strategy == "ITT")) 
