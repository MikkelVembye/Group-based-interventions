...
# DONE
timeline_plot <-  ggplot(G_timepllot, aes(x = year, fill = prereg)) + 
...
## Number of individuals in the total data
...
gb_dat |> 
  ...
  summarise(n = n(), .by = study) |> 
  summarise(n_studies = length(study), n_es = sum(n))
```

# Effect size distributions {.tabset}
...
density_reint_overall <- 
  reintergation_dat |> 
  mutate(es_group = "Overall - Reintegrational Outcomes") |> 
  ggplot(aes(gt_pop)) + 
  geom_density(fill = "cornflowerblue") + 
  geom_vline(data = reint_fig_dat, aes(xintercept = qrtl), linetype = "solid") + 
  geom_vline(data = reint_fig_dat, aes(xintercept = fence), linetype = "dashed") + 
  geom_rug(alpha = 0.25) + 
  facet_grid(~es_group) +
  theme_bw() + 
...
### Across reintegrational outcomes
...
reintegration_outcome_plot <- 
  ...
  geom_vline(data = reint_sub_fig_dat, aes(xintercept = qrtl), linetype = "solid")+
  geom_vline(data = reint_sub_fig_dat, aes(xintercept = fence), linetype = "dashed")+
  geom_rug(alpha = 0.25) +
