library(dplyr)
library(purrr)
library(stringr)
library(gt)

reintegration_dat <- 
  readRDS("reintegration_dat.rds") |> 
  mutate(
    schizophrenia = factor(schizophrenia, levels = c("Schizophrenia", "Other"))
  )

.study_avg <- function(data = reintegration_dat, category) {
  
  n_studies <- data |> pull(study) |> dplyr::n_distinct()
  n_es <- data |> nrow()
  
  res <- 
    data |> 
    group_by({{ category }}, study) |> 
    dplyr::summarise(
      n_es = dplyr::n(), 
      .groups = "drop"
    ) |> 
    group_by({{ category }}) |> 
    dplyr::summarise(
      J = n_distinct(study),
      K = sum(n_es),
      .groups = "drop"
    ) |> 
    mutate(
      perc_studies = (J/n_studies) * 100,
      perc_es = (K/n_es) * 100
    ) |> 
    rename(Category = {{ category }}, studies = J, effect_sizes = K)
  
  
  return(res)
  
}



x <- dplyr::vars(
  cnt,
  analysis_plan,
  schizophrenia,
  CBT_int,
  prereg_chr, 
  study_outlet, 
  QES_design,
  test_type,
  analysis_strategy,
  control,
  overall_rob,
  posttest_only,
  handle_multilevel
  ) 

pct_table_reint_dat <- 
  map_dfr(x, ~ .study_avg(category = !!.x)) |> 
  mutate(
    n = 1:n(),
    
    group = case_when(
      n %in% 1:15 ~ "Context",
      n %in% 16:24 ~ "Outcome measure",
      n %in% 25:26 ~ "Diagnosis in sample",
      n %in% 27:28 ~ "Intervention type",
      n %in% 29:30 ~ "Preregistration status",
      n %in% 31 ~ "Outlet",
      n %in% 32:33 ~ "Research design",
      n %in% 34:36 ~ "Test type",
      n %in% 37:38 ~ "Analysis strategy",
      n %in% 39:42 ~ "Control group",
      n %in% 43:45 ~ "Overall risk of bias",
      n %in% 46:47 ~ "Effect size metric",
      n %in% 48:49 ~ "Handles multilevel structure",
      .default = NA_character_
    )
  ) |> 
  relocate(group) |> 
  select(-n)

pct_table_reint <- 
  pct_table_reint_dat |> 
  gt(groupname_col = "group", rowname_col = "Category") |> 
  cols_label(
    Category = "Characteristics",
    studies = "Studies (J)",
    effect_sizes = "Effects (K)",
    perc_studies = "% Studies",
    perc_es = "% Effects"
  ) |> 
  tab_stub_indent(
    rows = everything(),
    indent = 3
  ) |> 
  tab_stubhead("Characteristics") |> 
  fmt_number(columns = 5:6, decimals = 1) |>  
  tab_options(
    table_body.hlines.color = "white",
    table_body.vlines.color = "white",
    table.border.left.color = "white",    # Removes the left border
    table.border.right.color = "white",   # Removes the right border
    table.font.size = 12
  )

pct_table_reint |> gtsave("Tables/pct_table_reint.docx")

  
.mean_table <- 
  function(
    var, char_name, digits,
    data = reintegration_dat, 
    group_var = study, 
    rm_mis = TRUE,
    ) {
    
    
    if (char_name != "Effect sizes per study"){
      stat <- 
        data |> 
        dplyr::group_by({{ group_var }}) |> 
        dplyr::summarise(
          kj = sum(!is.na({{ var }})),
          var_mean = round(mean({{ var }}, na.rm = rm_mis), digits = digits), 
          .groups = "drop"
        ) |> 
        dplyr::filter(kj > 0) |> 
        dplyr::summarise(
          
          Characteristic = char_name,
          
          N_studies = n(),
          N_ES = sum(kj),
          
          Mean_J = round(mean(var_mean, na.rm = rm_mis), digits = digits),
          SD_J = round(sd(var_mean, na.rm = rm_mis), 1),
          Range = paste0(min(var_mean, na.rm = rm_mis), "-", max(var_mean, na.rm = rm_mis)) 
          
        )
      
    } else {
      
      stat <- 
        data |> 
        dplyr::group_by({{ group_var }}) |> 
        dplyr::summarise(
          kj = n(),
          .groups = "drop"
        ) |> 
        dplyr::summarise(
          Characteristic = char_name,
          N_studies = n(),
          N_ES = sum(kj),
          
          Mean_J = round(mean(kj, na.rm = rm_mis), 1),
          SD_J = round(sd(kj, na.rm = rm_mis), 1),
          Range = paste0(min(kj, na.rm = rm_mis), "-", max(kj, na.rm = rm_mis)) 
        )
      
    }
    
    stat
    
  }


N_stud_trtid <- 
  reintegration_dat |> 
  reframe(
    n_es = n(), 
    N_treat = max(N_t),
    N_contr = max(N_c),
    .by = c(study, trt_id)
  ) 
 

.mean_table2 <- 
  function(
    var, 
    char_name,
    digits, 
    data = reintegration_dat,
    rm_mis = TRUE
  ){
    
    stats <- 
      data |> 
      dplyr::reframe(
        n_es = n(), 
        x = max({{ var }}, na.rm = rm_mis),
        .by = c(study, trt_id)
      ) |> 
      summarise(
        x_mean_stud = round(mean(x, rm.na = rm_mis), digits = digits),
        n_es = sum(n_es),
        .by = study
      ) |> 
      summarise(
        Characteristic = char_name,
        N_studies = n(),
        N_ES = sum(n_es),
        Mean_J = round(mean(x_mean_stud, rm.na = rm_mis), digits),
        SD_J = round(sd(x_mean_stud), 1),
        Range = paste0(round(min(x_mean_stud)), "-", round(max(x_mean_stud)))
      )
    
    stats
    
  }

.mean_table2(var = age_mean, char_name = "Avg. age", digits = 2) 

age_stud_trtid <- 
  reintegration_dat |> 
  reframe(
    n_es = n(), 
    N_treat = max(age_mean),
    .by = c(study, trt_id)
  )  

N_t_mean <- 
  N_stud_trtid |> 
  summarise(
    N_t_mean_stud = round(mean(N_treat)),
    n_es = sum(n_es),
    .by = study
  ) |> 
  summarise(
    Characteristic = "Raw sample size of treatment group",
    N_studies = n(),
    N_ES = sum(n_es),
    Mean_J = round(mean(N_t_mean_stud)),
    SD_J = round(sd(N_t_mean_stud), 1),
    Range = paste0(round(min(N_t_mean_stud)), "-", round(max(N_t_mean_stud)))
  )

N_c_mean <- 
  N_stud_trtid |> 
  summarise(
    N_c_mean_stud = round(mean(N_contr)),
    n_es = sum(n_es),
    .by = study
  ) |> 
  summarise(
    Characteristic = "Raw sample size of treatment group",
    N_studies = n(),
    N_ES = sum(n_es),
    Mean_J = round(mean(N_c_mean_stud)),
    SD_J = round(sd(N_c_mean_stud), 1),
    Range = paste0(round(min(N_c_mean_stud)), "-", round(max(N_c_mean_stud)))
  )

N_t_total <- 
  N_t_stud_trtid |> 
  summarise(N_es = sum(n_es), N_t_total = sum(N_treat), .by = study)

N_c_total <- 
  reintegration_dat |> 
  summarise(N_es = n(), N_c_total = max(N_c), .by = study)

N_total_dat <- 
  left_join(N_t_total, N_c_total, by = join_by(study, N_es)) |> 
  mutate(N_total = N_t_total + N_c_total)

.mean_table(data = N_total_dat, var = N_total, "Test")


# list all mean variables 

mean_vars <- dplyr::vars(
  N_t, N_c, N_total,
  ESS_t, ESS_c, ESS_total,
  age_mean,
  male_pct,
  total_number_of_sessions,
  sessions_per_week,
  duration_in_weeks,
  time_after_end_intervention_weeks,
  time_from_baseline_weeks
)

mean_var_names <- c(
  "Raw sample size of treatment group",
  "Raw sample size of control group",
  "Raw sample size (total)",
  "Effective sample size of treatment group",
  "Effective sample size of control group",
  "Effective sample size (total)",
  "Age",
  "% Males",
  "Total number of sessions",
  "Sessions per week",
  "Duration (in weeks)",
  "After end of intervention (in weeks)",
  "From baseline (in weeks)"
)

list_x <- tibble(
  var = mean_vars,
  char_name = mean_var_names,
  digits = rep(c(0,1), c(6,7)),
)

mean_table_dat_reint <- 
  pmap(.l = list_x, .f = .mean_table) |> 
  list_rbind()

# Average pre-posttest correlation reintegration

ppcor_dat_reint <- 
  reintegration_dat |> 
  filter(
    str_detect(
      ppcor_method, regex("Cal", ignore_case = TRUE), 
    ) |
      str_detect(
        ppcor_method, regex("From study", ignore_case = TRUE), 
      ) 
  ) |> 
  summarise(
    Characteristic = "Pre-posttest correlation (emp. calc)",
    N_studies = n_distinct(study),
    N_ES = n(),
    Mean_J = mean(ppcor),
    SD_J = sd(ppcor),
    Range = paste0(round(min(ppcor), 3), "-", round(max(ppcor), 3))
    
  )

mean_table_dat_reint <- 
  mean_table_dat_reint |> 
  bind_rows(ppcor_dat_reint) |> 
  mutate(
    group = rep(
      c("Sample characteristics", "Intervention characteristics", "Measurement timing", "Methodological features"), 
      c(8,3,2,1)
      )
  )

mean_table_reint <- 
  mean_table_dat_reint |> 
  gt(groupname_col = "group", rowname_col = "Characteristic") |> 
  cols_label(
    N_studies = "J",
    N_ES = "K",
    Mean_J = "Average",
    SD_J = "SD"
  ) |> 
  tab_stub_indent(
    rows = everything(),
    indent = 3
  ) |> 
  tab_stubhead("Characteristics") |> 
  fmt_number(columns = 4:5, decimals = 2) 

mean_table_reint |> gtsave("Tables/mean_table_reint.docx")
