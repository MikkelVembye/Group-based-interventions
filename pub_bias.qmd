...
hyema_outcome_tab <- 
  tibble(
    ...
    est_ci = c(
      ...
      NA_character_,
      as.character(round(sqrt(hyema_obj$tau2), 3)),
      as.character(n_distinct(reint_ma_dat$study)-1),
      as.character(nrow(reint_ma_dat)-3)
    ),
...
#W_mental <- chol2inv(chol(V_mat_mental))
#W_mental <- Matrix::solve(V_mat_mod_mental)
W_mental <- (V_mat_mental + t(V_mat_mental)) / 2
...
#W_prereg_mental <- chol2inv(chol(V_mat_prereg_mental))
W_prereg_mental <- (V_mat_prereg_mental + t(V_mat_prereg_mental)) / 2
...
W_notprereg_mental <- solve(V_mat_notprereg_mental)
