...
reintergation_dat <- 
  ...
  mutate(
    esid = 1:n(),
    outcome = case_when(
      str_detect(analysis_plan, "Alco|Hope|Social|Well", negate = TRUE) ~ "Other",
...
V_mat <- vcalc(vgt_pop, cluster = study, obs = esid, data = reintergation_dat, rho = rho)
...
sub_res <- 
  rma.mv(
    gt_pop ~ -1 + outcome,
    V = V_mat, 
    random = list(~ outcome | study, ~ outcome | esid),
    struct = c("DIAG", "DIAG"),
    data = reintergation_dat,
    sparse = TRUE
  )
...
# Mental health
V_mat_mental <- vcalc(vgt_pop, cluster = study, obs = esid, data = mental_health_dat, rho = rho)
...
y_lim_exp1 <- max(reintergation_dat$se_gt_pop) + 0.02 
...
# Egg CHE-ISCW-RVE ------------
V_mat_ISCW <- metafor::vcalc(vi = Wgt_pop, cluster = study, obs = esid, data = reintergation_dat, rho = rho)
