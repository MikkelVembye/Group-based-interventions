---
title: "Appendix A - Effect size calculation for 'Group-based community interventions to support
...
Gutman2019_est <-
  ...
  mutate(
    ...
    m_diff_c = if_else(outcome == "WHOQOL-BREF", m_post_c - m_pre_c, (m_post_c - m_pre_c) * -1),
  ...
  select(-c(SE_std, var_term1_reg)); Gutman2019_est
...
### **Halperin et al. [-@Halperin2000] (Quality-checked)**

```{r Halperin et al 2000}

Halperin2000 <- 
  tibble(
  outcome = c("BSPS", "SIAS", "CDSS", "GSI", "Q_LES_Q", "AUDIT"),
  ...
  sd_post_t = c(6.23, 11.89, 3.26, 10.59, 10.65, 5.68),
  ...
  sd_post_c = c(13.18, 11.39, 2.70, 5.75, 11.32, 9.24),
...
Halperin2000_est <- 
  ...
  mutate(
    ...
    ppcor_method = "F test values used",
  ...
  ungroup(); Halperin2000_est

```

```{r, echo=FALSE}
# Adding Halperin to data
dat <- 
  dat |> 
  bind_rows(Halperin2000_est) 
```
...
Schrank2016_est <-           
  ...
  mutate(
    ...
    # We don't use as these produce larger variance estimate relative to the
    # DiD variance estimator
    F_val = c(
      0.8,
      2.3,
      #  5.9,
...
Entering data from Table 2 and 3 (p. 505). We did not estimate the variance using the t-value for the social adjustment composite completer outcome, as it seemed to yield a too small variance estimate. 
...
Wojtalik2019_est <- 
  ...
  mutate(
    ...
    # ppcor imputed
    ppcor = if_else(
      analysis == "Completers" & str_detect(outcome, "Social"),
      R2_imp, # Using ppcor as subsitute for R2 as only one covariate is used
      NA_real_
    ),
    ...
    ppcor_method = 
      if_else(
        analysis == "Completers" & str_detect(outcome, "Social"), 
        "Imputed as R2 (too small variance estimate when using the t-val)", 
        "Not computable - t values used"
    )
  ...
  mutate(
    ...
    vd_adj = if_else(
      analysis == "Completers" & str_detect(outcome, "Social"),
      (1-ppcor^2) * (1/N_t + 1/N_c) + d_adj^2/(2*df_ind),
      d_adj^2/t_val^2 + d_adj^2/(2*df_ind)
    ),
    
    Wd_adj = if_else(
      analysis == "Completers" & str_detect(outcome, "Social"),
      (1-ppcor^2) * (1/N_t + 1/N_c),
      d_adj^2/t_val^2
    ),
    ...
    vg_adj = if_else(
      analysis == "Completers" & str_detect(outcome, "Social"),
      (1-ppcor^2) * (1/N_t + 1/N_c) + g_adj^2/(2*df_ind),
      g_adj^2/t_val^2 + g_adj^2/(2*df_ind)
    ),
  ...
  mutate(
    ...
    vgt_adj = if_else(
      analysis == "Completers" & str_detect(outcome, "Social"),
      (1-ppcor^2) * (1/N_t + 1/N_c) * adj_value_adj + gt_adj^2/(2*h_adj),
      vgt_adj
    ),
    
    Wgt_adj = if_else(
      analysis == "Completers" & str_detect(outcome, "Social"),
      (1-ppcor^2) * (1/N_t + 1/N_c) * adj_value_adj,
      Wgt_adj
    ),
    
    a = if_else(
      analysis == "Completers" & str_detect(outcome, "Social"),
      sqrt(2*Wgt_adj*df_adj),
      NA_real_
    ),
    
    # Cleaning the transformation value
    hg_adj = if_else(
      analysis == "Completers" & str_detect(outcome, "Social"),
       sqrt(2) * sign(gt_adj) * (log(abs(gt_adj) + sqrt(gt_adj^2 + a^2)) - log(a)),
      hg_adj
    ),
...
#D <- group_based_dat |> 
#  select(Author, test_name, vary_id, varifier)
