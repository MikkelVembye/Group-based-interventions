...
main_res_reint <- .PECHE_RVE(data = reint_ma_dat, pred_int = 95)
main_res_reint
...
#PI
c(main_res_reint$pi_lb_95, main_res_reint$pi_ub_95)
...
pi95_lb <- main_res_reint$pi_lb_95
pi95_ub <- main_res_reint$pi_ub_95

c(pi95_lb, pi95_ub)

# Heights of the curve at those bounds
y_pi <- approx(dens_df$x, dens_df$y, xout = c(pi95_lb, pi95_ub))$y
...
pi_plot_reint <- 
  ...
  geom_line() +
  annotate("segment",
           x = pi95_lb, xend = pi95_lb,
           y = 0, yend = y_pi[1],
           linetype = "dashed", linewidth = 0.6) +
  annotate("segment",
           x = pi95_ub, xend = pi95_ub,
           y = 0, yend = y_pi[2],
           linetype = "dashed", linewidth = 0.6) +
...
png("plots/predictive distribution reint.png", width = 8, height = 4, res = 300, unit = "in")
...
beta <- round(peche_res$avg_effect, 2)
cil <- round(peche_res$LL, 2)
ciu <- round(peche_res$UL, 2)
...
mean_label_dat <- 
  forest_dat2 |> 
  mutate(
    mean_es = round(peche_res$avg_effect, 2)
  )
...
main_res_mental <- .PECHE_RVE(data = mental_ma_dat, pred_int = 95)
main_res_mental
...
#PI
c(main_res_mental$pi_lb_95, main_res_mental$pi_ub_95)
...
#alpha <- 0.05
#tcrit <- qt(1 - alpha/2, df)
pi95_lb <- main_res_mental$pi_lb_95
pi95_ub <- main_res_mental$pi_ub_95
...
png("plots/predictive distribution mental.png", width = 8, height = 4, res = 300, unit = "in")
...
beta_mental <- round(peche_res_mental$avg_effect, 2)
cil_mental <- round(peche_res_mental$LL, 2)
ciu_mental <- round(peche_res_mental$UL, 2)
...
plot_mental <- 
  ...
  scale_x_continuous(breaks = seq(-3, 5, 0.5), limits = c(-1.5, 4.15)) +
  labs(
    x = "Hedges' g (95% CI)",
    color = "Type of outcome"
  ) +
