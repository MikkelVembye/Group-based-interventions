...
# MAIN RESULTS ARE MADE HERE
tic()
plan(multisession)
reint_cwb_res_update <- 
  pmap(
...
reint_cwb_res_update
#
saveRDS(reint_cwb_res_update, file = "Bootstrap results/reint_cwb_res_update.rds")

reint_cwb_res <- readRDS("Bootstrap results/reint_cwb_res_update.rds")

# Obtaining HTZ value for CBT intervention res
pmap(
    .l = arg_list_tbl_rho08[c(6, 12, 14),], 
    .f = .PESCE_RVE, 
    return_rma_obj = FALSE,
    CWB = FALSE
  )
...
while (!inherits(raw_res, "rma.mv") & i <= 4L) {
  ...
  raw_res <- tryCatch(
    suppressWarnings(
      metafor::rma.mv(
        gt_pop ~ CBT_intervention + alcohol_c + hope_c + lonely_c + self_est_c + 
          social_fun_c + wellbeing_c + other_c + schizo_c + prereg_c + clinical_c + 
...
raw_res |> clubSandwich::Wald_test(constraints = clubSandwich::constrain_equal(1:2), vcov = "CR2")
