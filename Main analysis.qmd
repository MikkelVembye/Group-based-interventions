...
Wald_test_cwb_fun <- 
  function(rma_fun_obj, seq_con, R = 19, seed = 080725){
    ...
    while (!inherits(cwb_res, "Wald_test_wildmeta") & i <= length(auxiliary_dist)) {
      cwb_res <- try( 
        Wald_test_cwb(
          ...
          constraints = constrain_equal(seq_con),
          R = R,
...
.SCE_RVE <- 
  function(formula, es, var, rand, structure, rho, data, model, table, return_rma_obj = FALSE){
...
plan(multisession)
x_test <- future_pmap(.l = arg_list_tbl_rho08[2,], .f = .SCE_RVE, return_rma_obj = TRUE, .options = opts); x_test
plan(sequential)

#plan(multisession)
#res_dat <- future_pmap(.l = arg_list_tbl, .f = .SCE_RVE, .options = opts) |> list_rbind(); res_dat
#plan(sequential)
...
main_res_table <- 
  ...
  cols_label(
    studies = "Studes",
    effects = "Effects",
    avg_effect_F_No  = html("Est [95% CI]<br>F stats"),
    pval_No = "Sig.",
    df_satt_No = "Satt. df",
    SD_total_No = html("SD total<br>&radic;&tau;<sup>2</sup>+&omega;<sup>2"),
    avg_effect_F_Yes  = html("Est [95% CI]<br>F stats"),
    pval_Yes = "Sig.",
    df_satt_Yes = "Satt. df",
