...
# For NHB paper Table 2 only

table2_func <- 
  function(filter_var) {
    gb_dat |> 
    dplyr::filter(.data[[filter_var]] == 1) |> 
    summarise(
    variable = filter_var,
...
names_mh_dis <- 
  gb_dat |> 
  select(schizophrenia_or_primary_psychotic_disorder:dissociative_identity_disorder) |> 
  names()

mental_health_dis <- 
  map(.x = names_mh_dis, .f = table2_func) |> 
  list_rbind() 

names_sm <- 
  gb_dat |> 
  select(other_social_problem:not_specified_social_problem) |> 
  names()

social_mar <- 
  map(.x = names_sm, .f = table2_func) |> 
  list_rbind()
...
# Save combined NHB Fig. 2 plot
#png(filename = "Figures/rob2_nhb_fig2.png", width = 12, height = 8, units = "in", res = 300)
rob_pct_effects_nhb / rob_pct_effects_mental_nhb
#dev.off()

## ROBINS-I plot for NHB paper
...
png(filename = "Figures/robinsi_reint_mental_nhb.png", width = 12, height = 4, units = "in", res = 600)
(robin_effects_nhb + robin_effects_mental_nhb ) +
  plot_layout(guides = "collect", axis_titles = "collect") & theme(legend.position = 'bottom') 
dev.off()
...
```{r es-per-study-vs-es-overall}
#| label: fig-n-es-vs-es-overall
#| fig-cap: "Distribution of number of effects per study vs. effect sizes for overall outcomes."
#| message: false
...
```{r es-per-study-vs-es-reint}
#| label: fig-n-es-vs-es-reint
#| fig-cap: "Distribution of number of effects per study vs. effect sizes for reintegrational outcomes."
#| fig-width: 9
#| fig-height: 6
#| message: false
...
```{r es-per-study-vs-es-mental}
#| label: fig-n-es-vs-es-mental
#| fig-cap: "Distribution of number of effects per study vs. effect sizes for mental health outcomes."
#| fig-height: 8
#| message: false
...
#| fig-cap: "Distribution of number of effects per study for reintegrational outcomes."
#| fig-width: 9
#| fig-height: 6
#| message: false
...
::: {.column width="95%"}
```{r effective-sample-size-overall-reint}
#| tbl-cap: "Distribution of primary study effective sample sizes at post test for reintegrational outcomes"
#| tbl-cap-location: top
#| label: tbl-sample-size-effective
...
::: {.column-margin} 
```{r effective-sample-size-overall-mental}
#| tbl-cap: "Distribution of primary study effective sample sizes at post test for mental health outcomes"
#| tbl-cap-location: top
#| label: tbl-sample-size-mental-effective
...
::: {.column width="95%"}
```{r sample-size-plot-reint-effective }
#| label: fig-sample-size-reint-effective
#| fig-cap: "Distribution of primary effective study sample sizes at post test for reintegrational outcomes."
#| fig.height: 2.5
#| fig.width: 8
#| fig.retina: 2
#| message: false
...
::: {.column-margin}
```{r sample-size-plot-mental-effective}
#| label: fig-sample-size-mental-effective
#| fig-cap: "Distribution of primary effective study sample sizes at post test for mental health outcomes."
#| fig.height: 4
#| fig.retina: 2
#| message: false
...
```{r outcome-dependency-table-all}
#| tbl-cap: "Dependency table across primary and secondary outcomes"
#| tbl-cap-location: top
#| label: tbl-outcome-all
...
#| tbl-cap: "Dependency table for reintegrational outcomes"
#| tbl-cap-location: top
#| label: tbl-outcome
...
```{r session-effect-size-plot-overall}
#| label: fig-session-effect-size-plot-overall
#| fig-cap: "Distribution of effect size across length of intervention and weight by the number of sessions per week for overall outcomes."
#| message: false
gb_dat |> 
  filter(!is.na(sessions_per_week)) |>  
...
::: {.column width="95%"}
```{r session-effect-size-plot-reint}
#| label: fig-session-effect-size-plot-reint
#| fig-cap: "Distribution of effect size across length of intervention and weight by the number of sessions per week for reintegrational outcomes."
#| fig-width: 9
#| fig-height: 6
#| message: false
...
```{r session-effect-size-plot-mental}
#| label: fig-session-effect-size-plot-mental
#| fig-cap: "Distribution of effect size across length of intervention and weight by the number of sessions per week for mental health outcomes."
#| fig-height: 8
#| message: false
...
```{r all-outcomes-forest}
#| label: fig-forest-all-outcome
#| fig-cap: "Forest plot including primary and secondary outcomes, respectively"
#| fig.width: 6.8
#| fig.height: 7
#| fig.retina: 2
#| fig.topcaption: TRUE
...
```{r all-outcomes-forest-nhb-only}
#| eval: false
#| include: false
#| echo: false

# For NHB paper Figure
...
study_ref_map <- c(
  "Acarturk et al. 2022" = "Acarturk et al.87",
  "Barbic et al. 2009" = "Barbic et al.88",
  "Bond et al. 2015" = "Bond et al.44",
  "Baekkelund et al. 2022" = "Bækkelund et al.116",
  "Cano-Vindel et al. 2021" = "Cano-Vindel et al.76",
  "Craigie & Nathan 2009" = "Craigie & Nathan77",
  "Crawford et al. 2012" = "Crawford et al.104",
  "Druss et al. 2010" = "Druss et al.96",
  "Druss et al. 2018" = "Druss et al.97",
  "Dyck et al. 2000" = "Dyck et al.98",
  "Gestel-Timmermans et al. 2012" = "van Gestel-Timmermans et al.119",
  "Gatz et al. 2007" = "Gatz et al.130",
  "Gonzalez & Prihoda 2007" = "Gonzalez & Prihoda90",
  "Gordon et al. 2018" = "Gordon et al.105",
  "Gutman et al. 2019" = "Gutman et al.91",
  "Hagen et al. 2005" = "Hagen et al.78",
  "Halperin et al. 2000" = "Halperin et al.79",
  "Haslam et al. 2019" = "Haslam et al.92",
  "Hilden et al. 2021" = "Hilden et al.106",
  "Himle et al. 2014" = "Himle et al.80",
  "Jacob et al. 2010" = "Jacob et al.113",
  "James et al. 2004" = "James et al.126",
  "Kanie et al. 2019" = "Kanie et al.107",
  "Lim et al. 2020" = "Lim et al.108",
  "Lloyd-Evans et al. 2020" = "Lloyd-Evans et al.131",
  "Madigan et al. 2013" = "Madigan et al.81",
  "McCay et al. 2006" = "McCay et al.99",
  "McCay et al. 2007" = "McCay et al.100",
  "Michalak et al. 2015" = "Michalak et al.82",
  "Morley et al. 2014" = "Morley et al.121",
  "Morton et al. 2012" = "Morton et al.118",
  "Patterson et al. 2003" = "Patterson et al.114",
  "Popolo et al. 2019" = "Popolo et al.94",
  "Rabenstein et al. 2016" = "Rabenstein et al.83",
  "Rosenblum et al. 2014" = "Rosenblum et al.122",
  "Russinova et al. 2018" = "Russinova et al.125",
  "Rusch et al. 2019" = "Rüsch et al.101",
  "Sacks et al. 2011" = "Sacks et al.129",
  "Sajatovic et al. 2009" = "Sajatovic et al.102",
  "Saloheimo et al. 2016" = "Saloheimo et al.103",
  "Schrank et al. 2016" = "Schrank et al.133",
  "Schafer et al. 2019" = "Schäfer et al.127",
  "Somers et al. 2017" = "Somers et al.128",
  "Tjaden et al. 2021" = "Tjaden et al.132",
  "Valiente et al. 2022" = "Valiente et al.134",
  "Volpe et al. 2015" = "Volpe et al.135",
  "Wojtalik et al. 2022" = "Wojtalik et al.109",
  "Wuthrich & Rapee 2013" = "Wuthrich & Rapee85",
  "Smith et al. 2021" = "Smith et al.84"
)
...
forest_dat <-
  ...
  mutate(
    ...
    #rma_mean = as.numeric(rma(gt, vgt, data = pick(dplyr::everything()))$b)
    rma_mean = round(yi, 2),
    study = recode(study, !!!study_ref_map)
  ) |> 
  arrange(rma_mean, study) |> 
  mutate(
    study = factor(study, levels = rev(unique(study)))
...
total_forest_gen_psy <- 
  forest_dat |>
  mutate(
    outcome_construct = case_when(
      str_detect(outcome_construct, "Reint") ~ "Socioal reintegration",
      str_detect(outcome_construct, "Men") ~ "Mental health ",
      TRUE ~ NA_character_
    )
  ) |>
  ggplot(
  ...
  annotate("rect", xmin = -Inf, xmax = 0, ymin = -Inf, ymax = 0.4, fill = "grey90") +
  annotate("rect", xmin = 0, xmax = Inf, ymin = -Inf, ymax = 0.4, fill = "grey90") +
  annotate("text", x = -0.75, y = 0, label = "Favors comparison", size = 3, color = "black") +
  annotate("text", x = 0.75, y = 0, label = "Favors group-based", size = 3, color = "black") +
  coord_cartesian(ylim = c(0.001, NA), clip = "off") +
  theme(plot.margin = margin(t = 5, r = 10, b = 20, l = 5)) +
  geom_vline(xintercept = 0, linetype = "dashed", color = "black", alpha = 0.5) 


png(filename = "Figures/forest_gen_psy.png", height = 11, width = 10, res = 600, units = "in")
total_forest_gen_psy
dev.off()

##
## Effects clearly different from zero (i.e., CIs not including zero) are:
#forest_dat |> filter(Est < 0 & CI_U < 0) |> summarise(n = n(), .by = c(study, outcome_construct))
...
```{r reint-mental-scatter-plot}
#| label: fig-outcome-scatter-reint-mental
#| fig-cap: "Scatter plot reintegration vs. mental"
#| fig.retina: 2
#| fig.topcaption: TRUE
#| message: false
